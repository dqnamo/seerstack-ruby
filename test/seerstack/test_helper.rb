...
class Seerstack::Test::SingletonClient < Seerstack::Client
  include Singleton

  TEST_API_BASE_URL = ENV.fetch("TEST_API_BASE_URL", "http://localhost:4010")

  def initialize
    super(base_url: Seerstack::Test::SingletonClient::TEST_API_BASE_URL, api_key: "My API Key")
  end
end
...
class Seerstack::Test::ResourceTest < Minitest::Test
  def async?
  ...
  def before_all
    super
    @seerstack = Seerstack::Test::SingletonClient.instance
  end
